...
import { removeRule } from './homeSlice';
...
export default function RuleCard({singleRule}) {
    ...
    return (
        <div className={"flex justify-between rounded border border-l-8 mb-2"+` border-${singleRule.color}-400`}>
            <p className={`p-2`}>
                "{singleRule.fieldName}" is {conditionOfRule} "{singleRule.value}"
            </p>
            <button onClick={removeTheRule} className="font-bold text-md border-l p-2 text-red-300" title="remove rule">x</button>
        </div>
    )
}
